...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        """
        if self.getLegalActions(state):
            actions = [action for action in self.getLegalActions(
                state) if self.computeValueFromQValues(state) == self.getQValue(state, action)]
    ...
    def getAction(self, state):
        ...
        action = None
        if legalActions:
            action = random.choice(legalActions) if util.flipCoin(
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature, count in features.items():
            self.weights[feature] += self.alpha * \
                ((reward + self.discount * value) - q_value) * count
