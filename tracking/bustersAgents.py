...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for beliefs in livingGhostPositionDistributions:
            bestBelief = max(beliefs, key=beliefs.get)
            #print(bestBelief)
            distance = self.distancer.getDistance(bestBelief, pacmanPosition)
            bestDis = min(distance, bestDis)
            if bestDis == distance:
                bestPos = bestBelief
        bestAct = Directions.STOP
        bestDist = None
        for a in legal:
            distance = self.distancer.getDistance(bestPos, Actions.getSuccessor(pacmanPosition, a))
            if bestDist == None:
            	bestDist = distance
            bestDist = min(distance, bestDist)
