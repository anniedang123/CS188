...
def cornersHeuristic(state, problem):
    ...
    for x in state[1]:
        if len(x) != 0:
            dist.append(util.manhattanDistance(state[0], x))
    if not dist:
        return 0
    return max(dist)
...
def foodHeuristic(state, problem):
    ...
    return max([mazeDistance(position, food_location, problem.startingGameState) for food_location in foodGrid.asList()]) if foodGrid.asList() else 0
